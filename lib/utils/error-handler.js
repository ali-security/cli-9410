...
var cbCalled = false
  ...
  , replaceInfo = require("./replace-info.js")
...
function errorHandler (er) {
  ...
  log.error("argv", args.map(JSON.stringify).join(" "))
  ...
  case "EEXIST":
    log.error([er.message
              ,"File exists: "+(er.dest || er.path)
              ,"Remove the existing file and try again, or run npm"
              ,"with --force to overwrite files recklessly."].join("\n"))
    break
